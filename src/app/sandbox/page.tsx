...
export default function Page() {
    return (
        <>
            <form action={async () => {
                "use server";
                const fo = await db.insert(folders_table).values(mockFolders.map((folder) => ({
                    id: parseInt(folder.id) + 1,
                    name: folder.name + Math.round(Math.random() * 1000),
                    parent: folder.parent === 'root' ? null : parseInt(folder.parent)
                })))

                const fi = await db.insert(files_table).values(mockFiles.map(file => ({
